...
console.log("MongoDB driver:", MongoInternals.defaultRemoteCollectionDriver());
  ...
  Meteor.methods({
    createImportSettingsTemplate: function (user, name, settings) {
      ...
      return Collections["magic.import.settings.templates"].insert(
        ...
        (error) => {
          console.log("create import", error);
        }
      );
    },
    ...
    renameImportSettingsTemplate: function (user, ID, name) {
      ...
      Collections["magic.import.settings.templates"].update(
        ...
        (error) => {
          console.log("rename import", error);
        }
      );
    },
    ...
    deleteImportSettingsTemplate: function (user, ID) {
      ...
      Collections["magic.import.settings.templates"].remove(
        ...
        (error) => {
          console.log("delete import", error);
        }
      );
    },
    ...
    getImportSettingsTemplates: function (user) {
      ...
      let templates = Collections["magic.import.settings.templates"]
        .find({ _user: user }, { sort: { _inserted: -1 } })
        .fetch();
    ...
    async magicGetPmagPyPlotFiles(cID, user, limit, attempt = 0) {
      ...
      try {
        ...
        console.error(
          "magicGetPmagPyPlotFiles",
          `Failed to retrieve PmagPy plot files for contribution ${cID}`,
          e
        );
    ...
    async magicGetPmagPyPlot(file, user, attempt = 0) {
      ...
      try {
        ...
        console.error(
          "magicGetPmagPyPlot",
          `Failed to retrieve PmagPy plot ${file}`,
          e
        );
        throw new Meteor.Error(
          "magicGetPmagPyPlot",
    ...
    async magicGetPublicContributions(ids, fileName, user, attempt = 0) {
      ...
      try {
        ...
        console.error(
          "magicGetPublicContributions",
          `Failed to retrieve private contributions for IDs ${ids.join(", ")}`,
          e
        );
