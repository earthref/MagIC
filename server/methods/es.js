...
export default function () {
  Meteor.methods({
    async esBuckets({ index, type, queries, countField, filters, aggs }) {
      // console.log("esBuckets", index, type, queries, filters, aggs);
      this.unblock();
      try {
        if (countField) {
          _.keys(aggs).forEach((aggKey) => {
            aggs[aggKey] = {
              ...aggs[aggKey],
              aggs: {
                count: {
        ...
        if (
          !_.find(
            _.map(queries, "terms"),
            "summary.contribution._private_key"
          ) &&
        ...
        if (countField) {
          _.keys(resp.body.aggregations).forEach((aggKey) => {
            resp.body.aggregations[aggKey].doc_count =
              resp.body.aggregations[aggKey].count.value;
            delete resp.body.aggregations[aggKey].count;
          });
        }
        return resp.body.aggregations.buckets
          ? _.reverse(
              _.sortBy(resp.body.aggregations.buckets.buckets, ["doc_count"])
            )
    ...
    async esCount({ index, type, queries, filters, countField, allVersions }) {
      ...
      try {
        ...
        if (_.isArray(queries)) search.query.bool.must.push(...queries);
        if (_.isArray(filters)) search.query.bool.filter.push(...filters);
        if (_.trim(countField) !== "")
          search.aggs = { count: { sum: { field: countField } } };
        ...
        let count =
          _.trim(countField) !== ""
            ? resp.body.aggregations.count.value
            : resp.body.count;
    ...
    async esPage(
      { index, type, queries, filters, source, sort },
      pageSize,
      pageNumber
    ) {
      //console.log(
      ...
      try {
        ...
        console.log(
          ...
          resp.body.hits.hits.map((hit) => hit._source).length
        );
    ...
    async esContributionIDs({ index, queries, filters }) {
      ...
      try {
        let search = {
          ...
          aggs: {
            buckets: { terms: { field: "summary.contribution.id", size: 1e6 } },
          },
        };
    ...
    async esCreatePrivateContribution({
      index,
      contributor,
      _contributor,
      name,
      ...
      try {
        ...
        if (
          isNaN(
            _.parseInt(
              next_id.body.hits.hits[0]._source.summary.contribution.id
            )
          )
        )
        ...
        contribution.contribution = (contribution.contribution &&
          ...
          contribution.contribution) || [{}];
        contribution.contribution[0] = _.merge(
    ...
    async esUpdatePrivateContribution({
      index,
      contributor,
      _contributor,
      id,
    ...
    async esUpdatePrivatePreSummaries({
      index,
      contributor,
      id,
      contribution,
      ...
      try {
        ...
        if (resp.body.hits.total.value > 0) {
          ...
          summary.contribution =
            resp.body.hits.hits[0]._source.summary.contribution;
        }
        //}

        await summarizer.preSummarizePromise(contribution, {
          summary: { contribution: summary.contribution },
        });
        ...
        let bulkIndex = [],
          rowIdx = 1;
        _.without(_.keys(summarizer.json), "contribution").forEach(
          (docType) => {
        ...
        await BPromise.map(
            ...
            return new Promise((resolve) => {
              const body = bulkIndexChunk.map((row) => {
                if (row && row.summarizer) {
                  ...
                  row =
                    summarizer.json[row.summarizer.docType][
                      row.summarizer.name
                    ][row.summarizer.parent];
                  row.summary = row.summary || {};
                  row.type = docType;
                  row.summary.contribution =
                    summarizer.json.contribution.summary.contribution;
                }
                return row;
              });
              ...
              esClient.bulk({ body }, (err, resp) => {
                if (!resp || resp.body.errors) {
                  console.error(
                    "esUpdatePrivatePreSummaries errors in chunk",
                    i + 1,
                    "of",
                    n,
                    JSON.stringify(resp).substring(0, 10000)
                  );
                  resolve(false);
                } else {
                  console.log(
                    "esUpdatePrivatePreSummaries finished chunk",
                    i + 1,
                    "of",
                    n
                  );
          ...
          if (!_.every(results, Boolean))
            throw new Meteor.Error(
              ...
              "Failed to upload private contribution."
            );
        });

        console.log(
          "esUpdatePrivatePreSummaries finished",
          index,
          contributor,
          id
        );
    ...
    async esUpdatePrivateSummaries({
      index,
      contributor,
      id,
      contribution,
      ...
      try {
        ...
        await summarizer.summarizePromise(contribution, {
          summary: { contribution: summary.contribution },
        });
        ...
        await esClient.update({
          ...
          body: {
            doc: {
              _incomplete_summary:
                summarizer.json.contribution._incomplete_summary,
          ...
          (bulkIndexChunk, i, n) => {
            return new Promise((resolve) => {
              ...
              esClient.bulk({ body }, (err, resp) => {
                if (!resp || resp.body.errors) {
                  console.error(
                    "esUpdatePrivateSummaries errors in chunk",
                    i + 1,
                    "of",
                    n,
                    JSON.stringify(resp).substring(0, 10000)
                  ...
                  console.log(
                    "esUpdatePrivateSummaries finished chunk",
                    i + 1,
                    "of",
                    n
                  );
        ...
        console.log(
          "esUpdatePrivateSummaries finished",
          index,
          contributor,
          id
        );
            ...
            query: {
              bool: {
                filter: [
                  ...
                  {
                    term: {
                      "summary.contribution._is_activated": activated
                        ? "true"
                        : "false",
                    },
                  },
    ...
    async esGetPrivateContributionSummary({ index, id, contributor }) {
      ...
      try {
        ...
        console.error(
          "esGetPrivateContributionSummary",
          index,
          id,
          contributor,
          error.message
    ...
    async esUpdateContributionDescription({ index, id, description }) {
      ...
      try {
        ...
        if (resp.body.hits.total.value > 0) {
          let _history =
            resp.body.hits.hits[0]._source.summary.contribution._history;
    ...
    // TODO: pass login token to authenticate changes
    async esUpdateContributionReference({
      index,
      id,
      contributor,
      _contributor,
      ...
      try {
        ...
        if (resp.body.hits.total.value > 0) {
          _history[0].version =
            parseInt(
              resp.body.hits.hits[0]._source.summary.contribution._history[0]
                .version
            ) + 1;
          _history.push(
            ...resp.body.hits.hits[0]._source.summary.contribution._history
    ...
    async esValidatePrivateContribution({ index, id, contributor }) {
      ...
      try {
        ...
        await validator.validatePromise(
          resp.body.hits.hits[0]._source.contribution
        );
    ...
    async esActivateContribution({ index, id }) {
      ...
      try {
        ...
        if (
          resp.body.hits.total.value > 0 &&
          resp.body.hits.hits[0]._source.summary
        )
          contributionSummary =
            resp.body.hits.hits[0]._source.summary.contribution;
        if (
          resp.body.hits.total.value > 0 &&
          resp.body.hits.hits[0]._source.summary.contribution._history.length >
            1
        ) {
          prev_id =
            resp.body.hits.hits[0]._source.summary.contribution._history[1].id;
      ...
      try {
        ...
        resp = await esClient.updateByQuery({
          ...
          body: {
            script: {
              source:
                "ctx._source.summary.contribution = params.contributionSummary",
    ...
    async esUploadActivatedContributionToS3({ index, id }) {
      ...
      try {
        ...
        const history =
          resp.body.hits.hits[0]._source.summary.contribution._history;
        const isLatest =
          resp.body.hits.hits[0]._source.summary.contribution._is_latest;
        ...
        if (isLatest === "true") {
            ...
            try {
              s3DeleteKeys({
                ...
                console.log(
                  ...
                  `Deleted ${v.id}/magic_contribution_${v.id}.txt`
                );
                s3DeleteKeys({
                  bucket: "magic-contributions",
                  keys: [`${v.id}/`],
          ...
          try {
            s3UploadObject({
              ...
              console.log(
                "esUploadActivatedContributionToS3",
                `Uploaded ${id}/magic_contribution_${id}.txt to magic-contributions`
              );
            });
          } catch (e) {
            console.log(
              "esUploadActivatedContributionToS3",
              `Error uploading latest ${v.id}`,
              e
            );
          }
        }
        try {
          s3UploadObject({
            ...
            console.log(
              "esUploadActivatedContributionToS3",
              `Uploaded ${id}/magic_contribution_${id}.txt to magic-activated-contributions`
            );
          });
        } catch (e) {
          console.log(
            "esUploadActivatedContributionToS3",
            `Error uploading ${v.id}`,
            e
          );
        }
      } catch (error) {
      ...
      try {
        await esClient.updateByQuery({
          ...
          body: {
            script: {
              source:
                'ctx._source.summary.contribution._is_activated = "false"',
            },
            query: {
              term: {
                "summary.contribution.id": id,
      ...
      try {
        ...
        if (user && session && session.id) {
          ...
          if (thisSession.length)
            user.session = user.session.forEach((x) =>
              x.id === session.id ? session : x
            );
    ...
    async esNextAvailableHandleFromEmail({ email }) {
      ...
      try {
        ...
        if (resp.body.hits.total.value === 0) return handle;
        for (x of [...Array(1000).keys()]) {
          ...
          if (resp.body.hits.total.value === 0) return handle + (x + 1);
        }
      } catch (error) {
        console.error("esNextAvailableHandleFromEmail", email, error.message);
        throw new Meteor.Error(
          "esNextAvailableHandleFromEmail",
          `Failed to find the next available handle for ${email}.`
        );
      }
    },
    ...
    async esNextAvailableUserID() {
      ...
      try {
        ...
        throw new Meteor.Error(
          "esNextAvailableUserID",
          "Failed to calculate the next available user ID."
        );
      }
    },
    ...
    async esCreateUserFromORCID({ name, email, orcid }) {
      ...
      try {
        ...
        if (email && email.address)
          handle = await Meteor.call("esNextAvailableHandleFromEmail", {
            email: email.address,
          });
        let user = {
          name,
    ...
    async esUpdateUserORCID({ id, name, email, orcid }) {
      ...
      try {
        ...
        throw new Meteor.Error(
          "esUpdateUserORCID",
          "Failed to update user ORCID data."
        );
      }
    },
    ...
    async esDisconnectUserORCID({ id }) {
      ...
      try {
        ...
        throw new Meteor.Error(
          "esDisconnectUserORCID",
          "Failed to disconnect user from ORCID."
        );
      }
    },
