...
class SearchSummariesListItem extends React.Component {
  ...
  renderTitle(item) {
    ...
    if (item.summary._all.location)
      title += " ⇒ " + item.summary._all.location[0];
  ...
  renderCounts(item) {
    ...
    let labels = [];
    let levels = [];
    ["Location", "Site", "Sample", "Specimen", "Experiment"].forEach(
      (label) => {
    ...
    if (
      item.summary &&
      item.summary._all &&
      item.summary._all._n_measurements
    ) {
    ...
    return paths.length > 0 ? (
      ...
      >
        {paths.length > 0 && (
          ...
          >
            <GoogleStaticMap width={150} height={150} paths={paths} />
          </a>
        )}
      </div>
    ) : (
    ...
    if (item?.row?.age && item?.row?.age_sigma) {
      return (
        ...
        >
          <b>Age:</b>
          <br />
          {item.row.age} ± {item.row.age_sigma} {item.row.age_unit}
        </div>
      );
      ...
      return (
        ...
        >
          <b>Age:</b>
          <br />
          {item.row.age_low} - {item.row.age_high} {item.row.age_unit}
        </div>
      );
      ...
      return (
        ...
        >
          <b>Age:</b>
          <br />
          {item.row.age} {item.row.age_unit}
        </div>
      );
    } else {
      return (
      ...
      return (
        ...
        >
          ...
          {item.row.pole_lat < 0
            ...
            : numeral(item.row.pole_lat).format("0[.]00") + "°N"}
          <br />
          {item.row.pole_lon < 0
          ...
          {item.row.pole_alpha95 &&
            numeral(item.row.pole_alpha95).format("0[.]00") + "°"}
        </div>
      );
    } else
      return (
      ...
      return (
        <div>
          <div
            ref="accordion"
            className={
              "ui accordion pole-view search-summaries-list-item" +
                          ...
                          return (
                            <tr key={i}>
                              <td style={{ whiteSpace: "nowrap" }}>
                                {v.id && v.id < 16282 && (
                                  <form
                                    action="//earthref.org/cgi-bin/z-download.cgi"
                                    method="post"
                                  >
                                    <input
                                      type="hidden"
                                      name="file_path"
                                      value={`/projects/earthref/local/oracle/earthref/magic/meteor/activated/magic_contribution_${v.id}.txt`}
                                    />
                                    <input
                                      type="hidden"
                                      name="file_name"
                                      value={`magic_contribution_${v.id}.txt`}
                                    />
                                    <button
                                      type="submit"
                                      className={
                                        "ui basic tiny fluid icon compact purple button"
                                      }
                                      style={{ marginTop: "0" }}
                                    >
                                ...
                                {!v.id && (
                                  <button
                                    className="ui basic tiny fluid compact icon purple disabled button"
                                    style={{ marginTop: "0" }}
                                  >
                                    <i className="ui file text outline icon" />{" "}
                                    Download
      ...
      >
        ...
        <Modal.Content>
          <div className="ui top attached tabular small menu search-tab-menu">
            ...
            this.state.contributionData.locations ? (
              <a
                className={`${
                  this.state.dataLevel === "locations" ? "active " : ""
                }item`}
                style={
                  this.state.dataLevel === "locations"
                  ...
                  this.setState(
                    this.state.dataEdited
                      ? {
                          ...
                          confirmChangeTabsDataLevel: "locations",
                        }
            ...
            {this.state.contributionData &&
            this.state.contributionData.sites ? (
              <a
                className={`${
                  this.state.dataLevel === "sites" ? "active " : ""
                }item`}
                style={
                  this.state.dataLevel === "sites"
                    ? { backgroundColor: "#F0F0F0" }
                    : {}
                }
                onClick={() =>
                  this.setState(
                    this.state.dataEdited
                      ? {
                          showConfirmChangeTabsEditedDataModal: true,
                          confirmChangeTabsDataLevel: "sites",
                        }
            ...
            {this.state.contributionData &&
            this.state.contributionData.samples ? (
              <a
                className={`${
                  this.state.dataLevel === "samples" ? "active " : ""
                }item`}
                style={
                  this.state.dataLevel === "samples"
                    ? { backgroundColor: "#F0F0F0" }
                    : {}
                }
                onClick={() =>
                  this.setState(
                    this.state.dataEdited
                      ? {
                          showConfirmChangeTabsEditedDataModal: true,
                          confirmChangeTabsDataLevel: "samples",
                        }
            ...
            {this.state.contributionData &&
            this.state.contributionData.specimens ? (
              <a
                className={`${
                  this.state.dataLevel === "specimens" ? "active " : ""
                }item`}
                style={
                  this.state.dataLevel === "specimens"
                    ? { backgroundColor: "#F0F0F0" }
                    : {}
                }
                onClick={() =>
                  this.setState(
                    this.state.dataEdited
                      ...
                      : {
                          dataLoading: true,
                          dataLevel: "specimens",
                          confirmChangeTabsDataLevel: "specimens",
                        }
                  )
                }
              >
    ...
    if (
      ...
      Meteor.call(
        "esGetContribution",
        {
          ...
          tables: ["locations", "sites", "samples", "specimens"],
        },
        (error, c) => {
