...
import SearchFiltersGeospatial from "/client/modules/common/containers/search_filters_geospatial";
...
import plateBoundaries from "/lib/modules/er/plate_boundaries.js"; // fixed default import

// build a lookup of plate geometries (skip empty geometries)
const plateGeometries = _.reduce(
  plateBoundaries.features,
  (acc, f) => {
    if (
      f &&
      f.properties &&
      ...
      acc[f.properties.Code] = f.geometry; // store geometry by plate code
    }
    return acc;
  },
  {}
...
const plateOptions = _.sortBy(
  _.map(plateGeometries, (geom, code) => ({
    code,
    name: plateBoundaries.features.find(
      (f) => f.properties && f.properties.Code === code
    ).properties.PlateName,
  })),
  [(x) => x.name]
);

const model = models[_.last(versions)];
const sortedTables = _.sortBy(
...
const searchTerms = {
  ...
  doi: {
    field: "summary.contribution._reference.doi.raw",
    processor: (x) => _.toUpper(_.trim(x)),
  },
  orcid: {
    field: "summary.contribution._reference.authors._orcid.raw",
    processor: (x) => x,
  },
};
/*const searchMatches = {
...
const searchSortOption = {
  name: "Most Relevant First",
  sort: [{ _score: "desc" }],
};
const sortOptions = [
  { name: "Most Relevant First", sort: [{ _score: "desc" }] },
  {
    name: "Recently Contributed First",
    sort: [{ "summary.contribution.timestamp": "desc" }],
  },
  {
    name: "Recently Contributed Last",
    sort: [{ "summary.contribution.timestamp": "asc" }],
  },
  {
    name: "Recently Published First",
    sort: [{ "summary.contribution._reference.year": "desc" }],
  },
  {
    name: "Recently Published Last",
    sort: [{ "summary.contribution._reference.year": "asc" }],
  },
  {
    name: "Most Cited Publication First",
    sort: [{ "summary.contribution._reference.n_citations": "desc" }],
  },
  {
    name: "Citations A-z",
    sort: [{ "summary.contribution._reference.citation.raw": "asc" }],
  },
  {
    name: "Citations z-A",
    sort: [{ "summary.contribution._reference.citation.raw": "desc" }],
  },
...
_.keys(methodCodes).forEach((group) => {
  methodCodes[group].codes.forEach((code) => {
    ...
    const prefixes = _.filter(
      _.range(1, code.code.length),
      (i) => code.code[i] === "-"
    ).map((i) => code.code.slice(0, i));
    ...
    prefixes.forEach((prefix) => {
      ...
      if (!prefixFound) missingPrefixs[prefix] = true;
    });
  });
});
...
class Search extends React.Component {
  filters = {
    "": [
      {
        render: this.renderPublicationYearFilter.bind(this),
        defaultOpen: true,
        name: "publicationYear",
      },
      {
        render: this.renderBucketsFilter.bind(this),
        name: "summary.contribution._reference.authors._name",
        title: "Author",
        term: "summary.contribution._reference.authors._name.raw",
        aggs: {
          buckets: {
            terms: {
              field: "summary.contribution._reference.authors._name.raw",
              size: 10000,
            },
          },
        },
      ...
      {
        render: this.renderBucketsFilter.bind(this),
        name: "summary.contribution._contributor",
        title: "Contributor",
        term: "summary.contribution._contributor.raw",
        aggs: {
          buckets: {
            terms: {
              field: "summary.contribution._contributor.raw",
              size: 10000,
            },
          },
        },
      ...
      {
        render: this.renderGeospatialFilter.bind(this),
        defaultOpen: true,
        name: "geospatial",
      },
      {
        render: this.renderGeospatialPlatesFilter.bind(this),
        name: "geospatialPlates",
        title: "Geospatial Plate Boundaries",
      },
      {
        render: this.renderAgeFilter.bind(this),
        defaultOpen: true,
        name: "age",
      },
      ...
      {
        render: this.renderIntensityFilter.bind(this),
        defaultOpen: false,
        name: "intensity",
      },
      {
        ...
        cv: _.flatMap(methodCodes, (group) => group.codes.map((x) => x.code)),
      },
      {
        render: this.renderBucketsFilter.bind(this),
        name: "summary._all.external_database_ids",
        title: "External Database",
        term: "summary._all.external_database_ids.key.raw",
        aggs: {
          buckets: {
            terms: {
              field: "summary._all.external_database_ids.key.raw",
              size: 10000,
            },
          },
        },
      ...
      {
        render: this.renderBucketsFilter.bind(this),
        name: "summary._all.software_packages",
        title: "Software Package",
        term: "summary._all.software_packages.raw",
        aggs: {
          buckets: {
            terms: { field: "summary._all.software_packages.raw", size: 10000 },
          },
        },
      },
      {
        render: this.renderBucketsFilter.bind(this),
        name: "summary.contribution.lab_names",
        title: "Lab Name",
        term: "summary.contribution.lab_names.raw",
        aggs: {
          buckets: {
            terms: { field: "summary.contribution.lab_names.raw", size: 10000 },
          },
        },
      },
      {
        render: this.renderBucketsFilter.bind(this),
        name: "summary.contribution.funding",
        title: "Funding",
        term: "summary.contribution.funding.raw",
        aggs: {
          buckets: {
            terms: { field: "summary.contribution.funding.raw", size: 10000 },
          },
        },
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withAnyData",
        title: "With Data in Any Table",
        summaryLevel: "_all",
        exists: true,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withoutAnyData",
        title: "Without Data in Any Table",
        summaryLevel: "_all",
        exists: false,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withLocationsData",
        title: "With Locations Table Data",
        summaryLevel: "locations",
        exists: true,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withoutLocationsData",
        title: "Without Locations Table Data",
        summaryLevel: "locations",
        exists: false,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withSitesData",
        title: "With Sites Table Data",
        summaryLevel: "sites",
        exists: true,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withoutSitesData",
        title: "Without Sites Table Data",
        summaryLevel: "sites",
        exists: false,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withSamplesData",
        title: "With Samples Table Data",
        summaryLevel: "samples",
        exists: true,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withoutSamplesData",
        title: "Without Samples Table Data",
        summaryLevel: "samples",
        exists: false,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withSpecimensData",
        title: "With Specimens Table Data",
        summaryLevel: "specimens",
        exists: true,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withoutSpecimensData",
        title: "Without Specimens Table Data",
        summaryLevel: "specimens",
        exists: false,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withMeasurementsData",
        title: "With Measurements Table Data",
        summaryLevel: "experiments",
        exists: true,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withoutMeasurementsData",
        title: "Without Measurements Table Data",
        summaryLevel: "experiments",
        exists: false,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withCriteriaData",
        title: "With Criteria Table Data",
        summaryLevel: "criteria",
        exists: true,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withoutCriteriaData",
        title: "Without Criteria Table Data",
        summaryLevel: "criteria",
        exists: false,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withAgesData",
        title: "With Ages Table Data",
        summaryLevel: "ages",
        exists: true,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withoutAgesData",
        title: "Without Ages Table Data",
        summaryLevel: "ages",
        exists: false,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withImagesData",
        title: "With Images Table Data",
        summaryLevel: "images",
        exists: true,
      },
      {
        render: this.renderWithDataFilter.bind(this),
        name: "withouImagestData",
        title: "Without Images Table Data",
        summaryLevel: "images",
        exists: false,
      },
    ],
    Poles: [
      ...
      {
        render: this.renderPoleAlpha95Filter.bind(this),
        name: "poleAlpha95",
      },
      {
        render: this.renderGeologicUnitsFilter.bind(this),
        name: "geologicUnits",
        title: "Geologic Units",
      },
      {
        render: this.renderBucketsFilter.bind(this),
        name: "summary._all.plate_blocks",
        title: "Plate Block",
        term: "summary._all.plate_blocks.raw",
        aggs: {
          buckets: {
            terms: { field: "summary._all.plate_blocks.raw", size: 10000 },
          },
        },
        cv: cvs.type.items.map((x) => x.item),
      },
      {
        render: this.renderBucketsFilter.bind(this),
        name: "summary._all.tectonic_settings",
        title: "Tectonic Setting",
        term: "summary._all.tectonic_settings.raw",
        aggs: {
          buckets: {
            terms: { field: "summary._all.tectonic_settings.raw", size: 10000 },
          },
        },
        cv: cvs.type.items.map((x) => x.item),
      },
  ...
  toggleFilter(filter) {
    ...
    if (
      ...
      this.state.activeBucketsFilters[filter.name]
    )
      return;
    ...
    const wasOpen =
      openedFilters[name] ||
      (openedFilters[name] === undefined && filter.defaultOpen);
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      selectedPlates: [], // added
      selectedGeologicUnitLabels: [],
      geologicUnitLabels: [],
    };
    this.styles = {
    ...
    this.handleNumericInput = _.debounce((input, value, min, max) => {
      ...
      if (value === "") this.setState({ [input]: undefined });
  ...
  componentDidMount() {
    ...
    // Preload geologic unit labels lazily
    // try {
    //   Meteor.call("geologicUnitsLabels", (err, labels) => {
    //     if (!err && Array.isArray(labels)) {
    //       this.setState({ geologicUnitLabels: labels });
    //     }
    //   });
    // } catch (e) {}
  }
  ...
  onWindowResize() {
    try {
      ...
      if (windowWidth !== this.windowWidth) {
        ...
        const width =
          windowWidth -
          $(this.refs["filters"]).outerWidth() -
          2 * $(this.refs["filters"]).offset().left -
          10;
      ...
      (match, term, value) => {
        queries.push(
          searchTerms[term]
            ? {
                terms: {
                  [searchTerms[term].field]: value
                    .split(/\s*\|\|\s*/)
                    .map((x) => searchTerms[term].processor(x)),
                },
              }
            : {
                wildcard: {
  ...
  getESFilters(excludeFilterName) {
    ...
    const esFilters = _.reduce(
      this.filters[activeView.name === "Poles" ? activeView.name : ""],
      (esFilters, filter) => {
        if (excludeFilterName === filter.name) return esFilters;
        if (this.state.activeBucketsFilters[filter.name])
          esFilters.push({
            terms: {
              [filter.term]: this.state.activeBucketsFilters[filter.name],
            },
          });
        ...
        if (this.state.activeNotExistsFilters[filter.name])
          esFilters.push({
            bool: {
              minimum_should_match: 1,
              should: [
                ...this.state.activeNotExistsFilters[filter.name].map(
                  (field) => {
                    return {
                      bool: {
                        must: {
                          exists: { field: `summary.${filter.summaryLevel}` },
                        },
    ...
    if (
      _.isNumber(this.state.lat_min) ||
      _.isNumber(this.state.lat_max) ||
      _.isNumber(this.state.lon_min) ||
      _.isNumber(this.state.lon_max)
    ) {
      let lon_min = -180;
      ...
      const lat_max = _.isNumber(this.state.lat_max)
        ? Math.min(90, Math.max(-90, this.state.lat_max))
        : 90;
      const lat_min = _.isNumber(this.state.lat_min)
        ? Math.min(90, Math.max(-90, this.state.lat_min))
        : -90;
      // Use a polygon bbox to avoid envelope parsing issues
      esFilters.push({
        geo_shape: {
    ...
    if (
      _.isNumber(this.state.pole_lat_min) ||
      _.isNumber(this.state.pole_lat_max) ||
      _.isNumber(this.state.pole_lon_min) ||
      _.isNumber(this.state.pole_lon_max)
    ) {
    ...
    if (
      _.isNumber(this.state.vgp_lat_min) ||
      _.isNumber(this.state.vgp_lat_max) ||
      _.isNumber(this.state.vgp_lon_min) ||
      _.isNumber(this.state.vgp_lon_max)
    ) {
      let vgp_lon_min = -180;
    ...
    if (_.isNumber(this.state.age_min) && _.isNumber(this.state.age_max))
      esFilters.push({
        range: {
          "summary._all._age_range_ybp.range": {
            gte: Math.min(
              _.find(ageUnits, {
                name: this.state.age_min_unit || ageUnitsDefault,
              }).from(this.state.age_min),
              _.find(ageUnits, {
                name: this.state.age_max_unit || ageUnitsDefault,
              }).from(this.state.age_max)
            ),
            lte: Math.max(
              _.find(ageUnits, {
                name: this.state.age_min_unit || ageUnitsDefault,
    ...
    if (_.isNumber(this.state.int_min) && _.isNumber(this.state.int_max))
      esFilters.push({
        range: {
          "summary._all.int_abs.range": {
            gte: Math.min(
              _.find(intUnits, {
                name: this.state.int_unit || intUnitsDefault,
              }).from(this.state.int_min),
              _.find(intUnits, {
                name: this.state.int_unit || intUnitsDefault,
              }).from(this.state.int_max)
            ),
            lte: Math.max(
              _.find(intUnits, {
                name: this.state.int_unit || intUnitsDefault,
    ...
    if (
      _.isNumber(this.state.pole_alpha95_min) &&
      _.isNumber(this.state.pole_alpha95_max)
    )
      esFilters.push({
        range: {
    ...
    if (
      excludeFilterName !== "geospatialPlates" &&
      this.state.selectedPlates &&
      ...
      if (plateShould.length > 0) {
        esFilters.push({
          bool: { minimum_should_match: 1, should: plateShould },
        });
      }
    }

    // Geologic units geo_shape filters (bbox polygons), similar to plates
    //if (
    //  excludeFilterName !== "geologicUnits" &&
    //  this.state.selectedGeologicUnitLabels &&
    //  this.state.selectedGeologicUnitLabels.length > 0
    //) {
    //  // Build should from cached shapes via server if needed
    //  // We'll optimistically assume label->shape from server method per request
    //  try {
    //    // Synchronous usage is not available; instead, store to temp on instance and refresh
    //    if (!this.geologicUnitShapesCache) this.geologicUnitShapesCache = {};
    //    const missing = this.state.selectedGeologicUnitLabels.filter(
    //      (l) => !this.geologicUnitShapesCache[l]
    //    );
    //    if (missing.length > 0) {
    //      // Kick off async fetch; results will cause a re-render that adds filters
    //      Meteor.call(
    //        "geologicUnitsShapesForLabels",
    //        missing,
    //        (err, shapes) => {
    //          if (!err && shapes) {
    //            this.geologicUnitShapesCache = {
    //              ...this.geologicUnitShapesCache,
    //              ...shapes,
    //            };
    //            this.forceUpdate();
    ...
    //      );
    //    }
    //    const should = this.state.selectedGeologicUnitLabels
    //      .map((l) => this.geologicUnitShapesCache[l])
    //      .filter(Boolean)
    //      .map((shape) => ({
    //        geo_shape: {
    //          "summary._all._geo_point": {
    ...
    //      }));
    //    if (should.length > 0) {
    //      esFilters.push({ bool: { minimum_should_match: 1, should } });
    //    }
    //  } catch (e) {}
    //}
      ...
      <div className="magic-search">
        ...
        <div className="ui top attached tabular menu level-tabs">
          {levels.map((level, i) => (
            <div
              key={i}
              className={
                (this.state.levelNumber === i ? "active " : "") + "item"
              }
              style={
                this.state.levelNumber === i
                  ? this.styles.activeTab
                  : this.styles.a
              }
          ...
          {Cookies.get(
            ...
            <div className="right menu">
              <div className="item" style={{ paddingRight: 0 }}>
                <Link
                  className={portals["MagIC"].color + " ui compact button"}
        ...
        >
          <div style={{ display: "flex", width: "100%" }}>
            <div
              className="ui labeled fluid action input"
            ...
            >
              ...
              <input
                ...
                onChange={(e) => {
                  const $buttons = $(this.refs["searchButton"]).add(
            ...
            {this.state.search.indexOf("private_key:") === -1 && (
              <SearchDownload
                className={portals["MagIC"].color + " ui basic button"}
          ...
          >
            <div>
              ...
              >
                <div>
                  <div
                    className="ui top attached tabular small menu"
                ...
                <div
                  ref="filters"
                  className="ui small basic attached segment"
                  style={this.styles.filters}
                >
                  <div>
                    {this.filters[
                      activeView.name === "Poles" ? activeView.name : ""
                    ].map((filter, i) =>
                      filter.render(searchQueries, filter, i)
                    )}
                  </div>
                </div>
              </div>
  ...
  renderPublicationYearFilter(searchQueries, filter, i) {
    const defaultOpen =
      this.state.openedFilters[filter.name] ||
      (this.state.openedFilters[filter.name] === undefined &&
        filter.defaultOpen);
    return (
      <div key={i + "_" + this.state.filtersTap} style={this.styles.filter}>
        <div className="ui small accordion">
          <div
            ref={filter.name + "_title"}
            className={"title" + (defaultOpen ? " active" : "")}
          ...
          >
            ...
            >
              <div
                className="ui small labeled input"
                style={{ display: "flex" }}
              >
                <div
                  className={
                    "ui input" +
                    (this.state.pub_yr_min === null ? " error" : "")
                  }
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      this.handleNumericInput(
                        ...
                        e.target.value,
                        -Infinity,
                        _.isNumber(this.state.pub_yr_max)
                          ? this.state.pub_yr_max
                          : moment().year()
                      );
                    }}
                  />
                ...
                <div
                  className={
                    "ui input" +
                    (this.state.pub_yr_max === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
                    ...
                    onChange={(e) => {
                      this.handleNumericInput(
                        "pub_yr_max",
                        e.target.value,
                        _.isNumber(this.state.pub_yr_min)
                          ? this.state.pub_yr_min
                          : -Infinity,
                        moment().year()
                      );
                    }}
          ...
          >
            ...
            >
              ...
              >
                ...
                <div
                  className={
                    "ui input" + (this.state.lat_min === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
                ...
                <div
                  className={
                    "ui input" + (this.state.lat_max === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
              ...
              >
                ...
                <div
                  className={
                    "ui input" + (this.state.lon_min === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
                ...
                <div
                  className={
                    "ui input" + (this.state.lon_max === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
          ...
          >
            ...
            >
              ...
              >
                <div
                  className={
                    "ui input" + (this.state.age_min === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
                    ...
                    onChange={(e) => {
                      this.handleNumericInput(
                        ...
                        e.target.value,
                        _.isNumber(
                          this.state.age_min_unit === "AD" && this.state.age_max
                        )
                          ? this.state.age_max
                          : 0,
                        _.isNumber(
                ...
                <div
                  className={
                    "ui input" + (this.state.age_max === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
                    ...
                    onChange={(e) => {
                      this.handleNumericInput(
                        ...
                        e.target.value,
                        _.isNumber(
                          this.state.age_max_unit !== "AD" && this.state.age_min
                        )
                          ? this.state.age_min
                          : 0,
                        _.isNumber(
            ...
            >
              ...
              >
                ...
                <div
                  className={
                    "ui input" +
                    (this.state.pole_lat_min === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
                ...
                <div
                  className={
                    "ui input" +
                    (this.state.pole_lat_max === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
              ...
              >
                ...
                <div
                  className={
                    "ui input" +
                    (this.state.pole_lon_min === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
                ...
                <div
                  className={
                    "ui input" +
                    (this.state.pole_lon_max === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
            ...
            >
              ...
              >
                <div
                  className={
                    "ui input" +
                    (this.state.pole_a95_min === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
                    ...
                    onChange={(e) => {
                      this.handleNumericInput(
                        ...
                        e.target.value,
                        0,
                        _.isNumber(this.state.pole_a95_max)
                          ? this.state.pole_a95_max
                          : Infinity
                      );
                    }}
                  />
                ...
                <div
                  className={
                    "ui input" +
                    (this.state.pole_a95_max === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
                    ...
                    onChange={(e) => {
                      this.handleNumericInput(
                        "pole_alpha95_max",
                        e.target.value,
                        _.isNumber(this.state.pole_a95_min)
                          ? this.state.pole_a95_min
                          : 0,
                        Infinity
                      );
                    }}
            ...
            >
              ...
              >
                ...
                <div
                  className={
                    "ui input" +
                    (this.state.vgp_lat_min === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
                ...
                <div
                  className={
                    "ui input" +
                    (this.state.vgp_lat_max === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
              ...
              >
                ...
                <div
                  className={
                    "ui input" +
                    (this.state.vgp_lon_min === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
                ...
                <div
                  className={
                    "ui input" +
                    (this.state.vgp_lon_max === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
          ...
          >
            ...
            >
              ...
              >
                <div
                  className={
                    "ui input" + (this.state.int_min === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
                ...
                <div
                  className={
                    "ui input" + (this.state.int_max === null ? " error" : "")
                  }
                  style={{ flexShrink: "1", minWidth: 20 }}
                >
                  <input
                    ...
                    onChange={(e) => {
                      this.handleNumericInput(
                        "int_max",
                        e.target.value,
                        _.isNumber(this.state.int_min) ? this.state.int_min : 0,
                        Infinity
                      );
                    }}
                  />
                ...
                >
                  <div className="text">
                    {this.state.int_unit || intUnitsDefault}
                  </div>
                  <i className="dropdown icon" />
                  <div className="menu">
                    {intUnits.map((unit, i) => (
    ...
    sortedTables.forEach((tableKey) => {
      if (
        ...
        (filter.summaryLevel === "_all" ||
          (filter.summaryLevel === "experiments" &&
        ...
        _.sortBy(
          ...
          if (
            !aggs[key] &&
            (!column.validations ||
              !column.validations.includes("downloadOnly()"))
          ) {
            labels.push({
              key,
              label: column.label,
              style:
                ...
                filter.summaryLevel !== "_all" &&
                ((column.validations &&
                  column.validations.includes("required()") && {
                    color: "red",
                  }) ||
                  (column.validations &&
                    column.validations.includes("recommended()") && {
                      color: "orange",
                    }) ||
                  {}),
            });
      ...
      <div key={i + "_" + this.state.filtersTap} style={this.styles.filter}>
        <SearchFiltersExists
          ...
          onChange={
            filter.exists
              ? (filters) => {
                  let activeExistsFilters = _.cloneDeep(
                    this.state.activeExistsFilters
                  );
                  if (filters && filters.length && filters.length > 0)
                    activeExistsFilters[filter.name] = filters;
                  else delete activeExistsFilters[filter.name];
                  this.setState({ activeExistsFilters });
                }
              : (filters) => {
                  let activeNotExistsFilters = _.cloneDeep(
                    this.state.activeNotExistsFilters
                  );
                  if (filters && filters.length && filters.length > 0)
                    activeNotExistsFilters[filter.name] = filters;
                  else delete activeNotExistsFilters[filter.name];
          ...
          onMouseOver={(x, n) => `
            ...
            and ${
              n === 1 ? "matches" : "match"
            } your current text search and your other filters.`}
        />
      </div>
    );
  }
    ...
    return (
      <div key={i + "_" + this.state.filtersTap} style={this.styles.filter}>
        <SearchFiltersBuckets
          ...
          onChange={(filters) => {
            let activeBucketsFilters = _.cloneDeep(
              this.state.activeBucketsFilters
            );
            if (filters && filters.length && filters.length > 0)
              activeBucketsFilters[filter.name] = filters;
            else delete activeBucketsFilters[filter.name];
  ...
  renderGeospatialPlatesFilter(searchQueries, filter, i) {
    const esFilters = this.getESFilters(filter.name);
    const activeView =
      _.find(levels[this.state.levelNumber].views, { name: this.state.view }) ||
      levels[this.state.levelNumber].views[0];
    ...
    const defaultOpen =
      this.state.openedFilters[filter.name] ||
      (this.state.openedFilters[filter.name] === undefined &&
        (filter.defaultOpen || false));

    // Build labels and aggregations keyed by label so the Exists container can read object-style aggs
    const labels = [];
    const aggs = {};
    plateOptions.forEach((p) => {
      ...
      if (geom) {
        ...
        aggs[key] = {
          filter:
            activeView && activeView.name === "Poles"
              ? {
                  bool: {
                    must: [
                      geoShapeFilter,
    ...
    // Helpers to map between UI keys and codes
    const keyFromCode = (code) => {
      const p = plateOptions.find((x) => x.code === code);
      return p ? `${p.name} (${p.code})` : code;
    };
    const codeFromKey = (key) => {
      const m = key.match(/\(([^)]+)\)\s*$/);
      return m ? m[1] : key;
    };
    ...
    return (
      <div key={i + "_" + this.state.filtersTap} style={this.styles.filter}>
        <SearchFiltersGeospatial
          ...
          activeFilters={this.state.selectedPlates.map(keyFromCode)}
          onChange={(filtersArr) => {
            const selectedPlates = (filtersArr || []).map(codeFromKey);
  ...
  renderGeologicUnitsFilter(searchQueries, filter, i) {
    const esFilters = this.getESFilters(filter.name);
    const activeView =
      _.find(levels[this.state.levelNumber].views, { name: this.state.view }) ||
      levels[this.state.levelNumber].views[0];
    ...
    // Build labels array from state (server-provided)
    const labels = this.state.geologicUnitLabels || [];

    // Build aggs object only for labels we have, but avoid sending huge aggs by default
    // Instead, request aggs for visible labels when the accordion is opened.
    const defaultOpen =
      this.state.openedFilters[filter.name] ||
      (this.state.openedFilters[filter.name] === undefined &&
        (filter.defaultOpen || false));

    const aggs = this.state["_geo_units_aggs"];
    if (
      defaultOpen &&
      !aggs &&
      labels &&
      labels.length > 0 &&
      !this._requestedGeoUnitsAggs
    ) {
      this._requestedGeoUnitsAggs = true;
      const labelKeys = labels.map((x) => x.key);
      Meteor.call("geologicUnitsAggsForLabels", labelKeys, (err, resp) => {
        if (!err) this.setState({ ["_geo_units_aggs"]: resp });
      });
    }
    ...
    return (
      <div key={i + "_" + this.state.filtersTap} style={this.styles.filter}>
        <SearchFiltersGeospatial
          ...
          activeFilters={this.state.selectedGeologicUnitLabels}
          onChange={(filtersArr) => {
            this.setState({
              selectedGeologicUnitLabels: filtersArr || [],
            });
          }}
          onClick={() => {
            let openedFilters = _.cloneDeep(this.state.openedFilters);
  ...
  renderTabs(searchQueries, esFilters) {
    ...
    return (
      <div
        ref="tabs"
        className="ui top attached tabular small menu search-tab-menu"
      >
        {levels[this.state.levelNumber].views.map((view) => (
          <div
            key={view.name}
            className={
              (activeView.name === view.name ? "active " : "") + "item"
            }
        ...
        <div className="right aligned item" style={{ padding: "0 1em" }}>
          <div
            ref="sort"
            className={portals["MagIC"].color + " ui dropdown label"}
            style={{ padding: "0.5em" }}
          >
            ...
            <div className="menu">
              {searchQueries.length > 0 && (
                <div
                  className={
                    ((searchQueries.length > 0 && this.state.sortDefault) ||
                    this.state.sort === searchSortOption.name
                      ? "active "
                      : "") + "item"
                  }
                >
              ...
              {searchQueries.length > 0 && <div className="divider" />}
              {sortOptions.map((option) => (
                <div
                  key={option.name}
                  className={
                    (!(searchQueries.length > 0 && this.state.sortDefault) &&
                    this.state.sort === option.name
                      ? "active "
                      : "") + "item"
                  }
                >
  ...
  renderView(searchQueries, esFilters) {
    let viewStyle = {
      ...
      height: this.state.height
        ? this.state.height - $(this.refs["tabs"]).outerHeight()
        : "100%",
    ...
    let es = _.extend({}, activeView.es, {
      ...
      sort:
        searchQueries.length > 0 && this.state.sortDefault
          ...
          : _.find(sortOptions, { name: this.state.sort }).sort,
    });
    if (activeView.name === "Summaries")
      return (
        <SearchSummariesView
    ...
    if (activeView.name === "Rows")
      return (
        <SearchRowsView
          ...
          es={es}
          table={
            activeView.es.type === "experiments"
              ? "measurements"
              : activeView.es.type
          }
          pageSize={20}
    ...
    if (activeView.name === "Rock Magnetism")
      return (
        <SearchRMagView
          key={this.state.levelNumber + "_" + activeView.name}
          style={viewStyle}
    ...
    if (activeView.name === "Images" || activeView.name === "Plots")
      return (
        <SearchImagesView
          key={this.state.levelNumber + "_" + activeView.name}
          style={viewStyle}
