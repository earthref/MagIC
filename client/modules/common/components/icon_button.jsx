...
class IconButton extends React.Component {
  ...
  renderChildren() {
    let hasSubTitle = _.some(
      _.map(
        this.props.children,
        (c) => c && c.props && c.props.className === "subtitle"
      )
    );
        ...
        {React.Children.map(this.props.children, (child, i) => {
          if (child.props.className === "title")
            child = React.cloneElement(child, {
              className: "ui header " + this.portalColor(),
              style: headerStyle,
            });
          if (child.props.className === "small title")
            child = React.cloneElement(child, {
              className: "ui small header " + this.portalColor(),
              style: headerStyle,
            });
          if (child.props.className.indexOf("statistic") !== -1)
            child = React.cloneElement(child, {
              className: child.props.className + " " + this.portalColor(),
            });
          if (child.props.className === "subtitle")
            child = React.cloneElement(child, {
              className: "ui sub header",
              style: this.styles.subHeader,
            });
          return child;
        })}
      </div>
  ...
  render() {
    ...
    className =
      "ui icon header basic fluid button " +
      this.portalColor() +
      (disabled ? " disabled " : " ") +
      className +
      " er-icon-button";
