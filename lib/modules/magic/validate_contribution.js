          ...
          if (table === 'measurements') {
            _.keys(model.columns).forEach(column => {
              if (_.includes(model.columns[column].validations, "required()") && 
                ...
                this._addValidationError(table, column, `The ${table} table is missing required column "${column}".`);
            });
            ...
            this.json[table].forEach((row, idxRow) => {
              _.keys(model.columns).forEach(column => {
                if (_.includes(model.columns[column].validations, "required()") && _.isEmpty(row[column]))
                  this._addValidationError(table, column, `The ${table} table is missing required column "${column}".`, idxRow+1);
              });
            });
          } 
        }
