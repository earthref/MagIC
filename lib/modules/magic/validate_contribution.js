...
// Split a List/Dictionary/Matrix cell on its delimiter, ignoring delimiters
// inside double quotes. This used to be
//   val.split(/:(?=([^"]*"[^"]*")*[^"]*$)/)
// whose lookahead rescans the rest of the string at every delimiter, so a cell
// with n delimiters costs O(n * length). A specimens.experiments cell of ~1M
// characters with 60k colons (contribution 20428, 2026-09-10) took hours of CPU
// on the server's single thread and froze every other request. This walk is
// linear and splits at exactly the same positions: a delimiter is a separator
// when the text after it contains an even number of quotes.
function splitOutsideQuotes(str, delim) {
  str = String(str);
  let quotesAfter = 0;
  for (let i = 0; i < str.length; i++) if (str[i] === '"') quotesAfter++;
  const parts = [];
  let start = 0;
  for (let i = 0; i < str.length; i++) {
    const c = str[i];
    if (c === '"') quotesAfter--;
    else if (c === delim && quotesAfter % 2 === 0) {
...
// Iterate rows in chunks, yielding to the event loop between chunks. The
// validator runs inside a Meteor method on the server, where a synchronous
// walk over a large table (a 90k-row measurements table takes tens of
// seconds) blocks every other request on the single thread. Also used in the
// browser by the upload tool, hence the setTimeout fallback.
const yieldToEventLoop = typeof setImmediate === 'function'
  ? (fn) => setImmediate(fn)
  : (fn) => setTimeout(fn, 0);
function eachRowYielding(rows, fn, chunkSize = 50) {
  return new Promise((resolve, reject) => {
    let i = 0;
    const step = () => {
      try {
        const end = Math.min(i + chunkSize, rows.length);
        for (; i < end; i++) fn(rows[i], i);
      } catch (e) { return reject(e); }
      if (i < rows.length) yieldToEventLoop(step); else resolve();
    };
    step();
  });
}
...
export default class extends Runner {
        ...
        if (_.find(models[_.last(versions)].tables[table].columns[column].validations, x => { 
          ...
          if (this.json[table]) {
            if (table === 'measurements') {
              ...
              } else if (Array.isArray(this.json[table])) {
  ...
  _validateTables() {
    ...
    // Tables are validated in order, each row loop yielding to the event
    // loop between chunks (see eachRowYielding) so a large contribution
    // cannot freeze the server for the duration of its validation.
    return Promise.each(sortedTables, (table, tableIdx) => {

      if (!this.json[table]) {
        if (table === 'locations' || table === 'sites') {
          this._addValidationError(table, 'contribution_id', `The ${table} table is required.`, 1);
        }
        return Promise.resolve();
      }
      ...
      if (table === 'measurements' && this.json[table].rows) {
        // Tabular format: only check required columns when rows actually exist
        if (this.json[table].rows.length > 0) {
          _.keys(model.columns).forEach((column) => {
            if (_.includes(model.columns[column].validations, 'required()') &&
              ...
              this.validation.errors[table][column][`The ${table} table is missing required column "${column}".`] = {};
            }
          });
        }
        if (this.validation.errors[table]) return Promise.resolve();
        const columns = this.json[table].columns;
        return eachRowYielding(this.json[table].rows.slice(0, 10000), (row, idxRow) => {
          row.forEach((val, colIdx) => {
            if (colIdx >= columns.length) {
              this._addValidationError(table, columns[columns.length - 1],
                `The ${table} table row has more values than the column headers.`, idxRow+1);
            } else {
              this._validateColumn(table, columns[colIdx], columns, idxRow, val, _.zipObject(columns, row));
            }
          });
        });
      }

      // Array-of-objects format (every other table, and measurements when
      // not tabular): validate every model column of every row.
      if (!Array.isArray(this.json[table])) return Promise.resolve();
      const modelColumns = _.keys(model.columns);
      return eachRowYielding(this.json[table], (row, idxRow) => {
        const rowColumns = _.keys(row);
        modelColumns.forEach(column => {
          this._validateColumn(table, column, rowColumns, idxRow, row[column], row);
        });
      });
  ...
  _validateColumn(table, column, columns, idxRow, val, row) {
    ...
    if (_.trim(column) === '' && _.trim(val) !== '') {
      this._addValidationError(table, column, 
        `The ${table} table has data in a column missing a name.`, idxRow+1);
    }
    ...
    if (_.includes(model.columns[column].validations, "required()") && 
      ...
      this._addValidationError(table, column, 
        `The ${table} table is missing data in required column "${column}".`, idxRow+1);
    }
    ...
    if (row && _.find(model.columns[column].validations, x => { return requiredIf = x.match(/^requiredIf\("(.*)"\)$/); }) &&
      ...
      this._addValidationError(table, column, 
        `The ${table} table is missing data in required column "${column}" since column "${requiredIf[1]}" is not empty.`, idxRow+1);
    }
    ...
    if (row && _.find(model.columns[column].validations, x => { return requiredIfGroup = x.match(/^requiredIfGroup\("(.*)"\)$/); }) &&
      ...
      this._addValidationError(table, column, 
        `The ${table} table is missing data in required column "${column}" since group "${requiredIfGroup[1]}" is not empty.`, idxRow+1);
    }
    ...
    if (row && _.find(model.columns[column].validations, x => { return requiredUnless = x.match(/^requiredUnless\("(.*)"\)$/); }) &&
      ...
      this._addValidationError(table, column, 
        `The ${table} table is missing data in required column "${column}" since column${nRequired === 1 ? '' : 's'} "${requiredUnless[1]}" ${nRequired === 1 ? 'is' : 'are'} empty.`, idxRow+1);
    }
    ...
    if (row && _.find(model.columns[column].validations, x => { return requiredUnlessTable = x.match(/^requiredUnlessTable\("(.*)"\)$/); }) &&
      ...
      this._addValidationError(table, column, 
        `The ${table} table is missing data in required column "${column}" since there is no "${requiredUnlessTable[1]}" table.`, idxRow+1);
    }

    let vals = [val];
    if (val && (model.columns[column].type === 'List' || model.columns[column].type === 'Dictionary'))
      vals = _.filter(splitOutsideQuotes(val, ':'), x => _.trim(x) !== '');
    if (val && model.columns[column].type === 'Matrix')
      vals = _.filter(splitOutsideQuotes(val, ';'), x => _.trim(x) !== '');

    vals.forEach(v => {
      v = _.trim(v);
      if (v === undefined || v === null || v === '') return;
      if (model.columns[column].type === 'Dictionary') v = v.split("[", 1)[0];
      if (model.columns[column].type === 'Matrix') v = v.split(":", 1)[0];

      if (model.columns[column].type === 'Integer' && !/^[+-]?\d+$/.test(v)) {
        this._addValidationError(table, column, 
          `The ${table} table column "${column}" value "${v}" is not of type "Integer".`, idxRow+1);
      }
        
      if (model.columns[column].type === 'Number' && !/^[+-]?(\d*\.?\d+|\d+\.)(e[+-]?\d*\.?\d+)?$/i.test(v)) {
        this._addValidationError(table, column, 
          `The ${table} table column "${column}" value "${v}" is not of type "Number".`, idxRow+1);
      }

      if (model.columns[column].type === 'Timestamp' && !moment(v).isValid()) {
        this._addValidationError(table, column, 
          `The ${table} table column "${column}" value "${v}" is not of type "Timestamp".`, idxRow+1);
      }
      ...
      let cv;
      if (model.columns[column].type !== 'Matrix' && model.columns[column].type !== 'Dictionary' && _.find(model.columns[column].validations, x => { return cv = x.match(/^cv\("(.*)"\)$/); }) &&
          cvs[cv[1]] && !_.find(cvs[cv[1]].items, x => x.item === v)) {
        this._addValidationError(table, column, 
          `The ${table} table column "${column}" value "${v}" is not in the "${cvs[cv[1]].label}" controlled vocabulary.`, idxRow+1);
      }
      if (model.columns[column].type === 'Matrix' && _.find(model.columns[column].validations, x => { return cv = x.match(/^cv\("(.*)"\)$/); }) &&
          cvs[cv[1]] && !_.find(cvs[cv[1]].items, x => x => x.item === v)) {
        this._addValidationError(table, column, 
          `The ${table} table matrix column "${column}" value "${v}" is not in the "${cvs[cv[1]].label}" controlled vocabulary.`, idxRow+1);
      }
      if (model.columns[column].type === 'Dictionary' && _.find(model.columns[column].validations, x => { return cv = x.match(/^cv\("(.*)"\)$/); }) &&
          cvs[cv[1]] && !_.find(cvs[cv[1]].items, x => x => x.item === v)) {
        this._addValidationError(table, column, 
          `The ${table} table dictionary column "${column}" value "${v}" is not in the "${cvs[cv[1]].label}" controlled vocabulary.`, idxRow+1);
      }
