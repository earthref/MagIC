...
let reNSummaryColumn = new RegExp(/^\_n(\_|$)/);
...
export default class extends Runner {
  ...
  // Return a promise for creating the contribution summary
  summarizeContributionPromise(contribution, meta) {

    console.log('summarizeContributionPromise');
    ...
    return this._summarizeTables()
      .then(this._getCrossRefData.bind(this))
      .then(this._adoptContributionChildTables.bind(this))
      .then(this._aggregateTables.bind(this))
      .then(this._consolidate.bind(this));
          ...
          if (table === 'contribution') {
            ...
            this.contribution.measurements.rows.forEach((row) => {
              if (row[experimentColumnIdx]) {
                ...
                this._summarizeRowProps(rowObject, this.json.experiments[prop][parentProp].summary.experiments, model, onlyCounts);
            ...
            this.contribution[table].forEach((row) => {
              ...
              if (name !== '') {
                ...
                this._summarizeRowProps(row, this.json[joinTable][prop][parentProp].summary[table], model, onlyCounts);
            ...
            this.contribution.criteria.forEach((row, criteriaRowIdx) => {
              ...
              if (_.trim(row.criterion) !== '' && _.indexOf(_.keys(models[_.last(versions)].tables), criterionTable.toLowerCase()) >= 0) {
                ...
                this._summarizeRowProps(row, this.json.contribution.summary.criteria, model, onlyCounts);
                ...
                _.keys(this.json[criterionTable]).forEach((prop) => {
                  _.keys(this.json[criterionTable][prop]).forEach((parentProp) => {
                    ...
                    this._summarizeRowProps(row, this.json[criterionTable][prop][parentProp].summary.criteria, model, onlyCounts);
            ...
            this.contribution[table].forEach((row) => {
              ...
              this._summarizeRowProps(row, this.json[table][prop][parentProp].summary[table], model, onlyCounts);
  ...
  _summarizeRowProps(row, summary, model, onlyCounts) {
    ...
    if (!onlyCounts) ['age', 'age_low', 'age_high'].forEach((column) => {
    ...
    if (!onlyCounts && row.age_unit && row.age_sigma) {
    ...
    if (!onlyCounts && row.lon_w && row.lat_n && row.lon_e && row.lat_s) {
    ...
    let tableKeyColumns = { location: true, site: true, sample: true, specimen: true, experiment: true };
    _.keys(row).forEach((column) => {
      if (!model.columns[column]) {
        this._appendWarning(`Unrecognized data model column "${column}".`);
      } else {
        if (onlyCounts) {
          if (tableKeyColumns[column]) {
            this._initProp(summary, column, {});
            summary[column][_.trim(row[column])] = true;
          }
  ...
  _consolidateSummary(summary) {
    _.keys(summary).forEach(table => {
      _.keys(summary[table]).forEach(column => {
        if (_.isArray(summary[table][column]) && summary[table][column].length > 0 && _.isPlainObject(summary[table][column][0])) { 
        } else if (_.isPlainObject(summary[table][column]) && column[0] !== '_') {
          summary[table][column] = _.keys(summary[table][column]).slice(0,100);
        } else if (_.isArray(summary[table][column]) && (column[0] !== '_' || 
            reAgeSummaryColumn.test(column))) {
          summary[table][column] = {
            vals: summary[table][column].slice(0,100),
            n: summary[table][column].length,
            range: {
              gte: _.min(_.without(summary[table][column], undefined)),
              lte: _.max(_.without(summary[table][column], undefined))
            }
          };
        }
      });
        ...
        if (onlyCounts !== true) {
          if (model.columns[column]) {
            if (model.columns[column].type === 'Number' ||
              ...
              this._initProp(to.summary[toName], column, {});
              _.keys(from.summary[fromName][column]).forEach(v => to.summary[toName][column][v] = true);
            }
          }
