...
export default class extends Runner {
  ...
  summarizePromise(contribution, meta) {
    ...
    return this.preSummarizePromise(contribution, meta)
  ...
  _adoptContributionChildTables() {
    return this._adoptChildTables('contribution');
  }

  _adoptChildTables(toTable) {
  ...
  _summarizeRowProps(row, summary, model) {
    ...
    ['age', 'age_low', 'age_high'].forEach((column) => {
      if (row.age_unit && row[column]) {
        ...
        if (age !== undefined) {
          ...
          if (row.age_unit === 'Ka'                ) age_ybp = 1e3*age;
    ...
    if (row.age_unit && row.age_sigma) {
      ...
      if (age_sigma !== undefined) {
        ...
        if (row.age_unit === 'Ka'                ) age_sigma_y = 1e3*age_sigma;
