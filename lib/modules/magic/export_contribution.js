    ...
    for (let modelTable in this.model.tables) {
      ...
      for (let rowIdx = 0; rowIdx < (jsonToExport[modelTable].rows || jsonToExport[modelTable] || []).length; rowIdx++) {
        ...
        wsData.push(wsRow.map((value, i) => {
          if (Array.isArray(value) && value.length > 0 && !Array.isArray(value[0])) value = value.map(x => x && x.trim && x.trim() || x).join(':');
          return value && value.trim && value.trim() || value;
        }));

        /*if (rowIdx >= 50000) {
          wsData.push(['Warning! Excel exports are currently limited to 50,000 rows. Please refer to the MagIC Text File export for all of the rows.']);
